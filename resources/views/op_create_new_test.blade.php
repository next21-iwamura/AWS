	...
	<div>
		...
		<div class="title">&nbsp;&nbsp;&nbsp;テスト OP 新規商品アップ用 CSV 作成ツール</div>
			<div style="width:100%; margin:0 auto;">
					...
					@endif
					<!--<form action="/laravel/op_create_new_test" method="POST" onSubmit="return double()" name="prid">-->
					<form action="/op_create_new_test" method="POST" onSubmit="return double()" name="prid">
						{{ csrf_field() }}
		...
		@if(isset($create))
			...
			<div style="background:#f5f5f5; width:60%; display:flex;justify-content:space-between; flex-wrap: wrap; margin:0 auto 50px auto;">
				...
				@if(isset($csv_exists_flag3) && $csv_exists_flag3 == 'ON')<input type = "submit" class="s_button" id="dl6" name="dl6" value="ジャック商品ページおすすめ商品表示用CSV">@endif
