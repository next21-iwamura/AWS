...
<head>
    ...
    <title>売上分析ツール用・データアップロード</title>
...
<body>
    ...
    <div class="container">
        <div style="width:100%; color:#666666; text-align:center;padding:.5em 0;font-weight:bold;letter-spacing:.3em;">売上分析ツール用・データアップロード</div>
        <div>

        <!-- <form method="POST" action="/laravel/analysis" accept-charset="UTF-8" enctype="multipart/form-data">-->
       <form method="POST" action="/analysis" accept-charset="UTF-8" enctype="multipart/form-data">
             ...
             <div style="width:1280px;display:flex;justify-content:space-around;margin:10px auto 0 auto;border-top:3px double #d3d3d3;padding-top:30px">
                    @if (empty( $db_insert ) && empty( $search ) && empty( $search ) && empty( $create_info ) && empty( $create_info2)  && empty( $create_csv)  && empty( $dl ))
                        <div style="width:480px;border:1px dotted #d3d3d3;padding:20px 40px;background:#ffffff;">
                                    ...
                                    <p class="file_view">
                                        <label for="loadFileName2">ファイル選択</label>
                                        <input type="file" name="loadFileName2" value="ファイル選択" id="loadFileName2">
                                        <span class="select-image">ファイルが選択されていません</span>
                                    </p>
                                    <p class="file_view">
                                        <label for="loadFileName3">ファイル選択</label>
                                        <input type="file" name="loadFileName3" value="ファイル選択" id="loadFileName3">
                                        <span class="select-image">ファイルが選択されていません</span>
                                    </p>
                            ...
                            <!-- 「フォームの表示」ボタン押下時に足りないファイルなどのエラーがあった場合のコメントを表示 -->
                                ...
                                @else
                                            ...
                                            @if (isset( $db_insert ) && $db_insert == "ON")
                                                ...
                                                <p style="text-align:center;">
                                                @if (isset( $up_file_name ) && $up_file_name <> "")
                                                <input type = "hidden" name="up_file_name" value="{{ $up_file_name }}">
                                                @endif
                                                @if (isset( $up_file_name2 ) && $up_file_name2 <> "")
                                                <input type = "hidden" name="up_file_name2" value="{{ $up_file_name2 }}">
                                                @endif
                                                @if (isset( $up_file_name3 ) && $up_file_name3 <> "")
                                                <input type = "hidden" name="up_file_name3" value="{{ $up_file_name3 }}">
                                                @endif
